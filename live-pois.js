(() => {
  ...
  const CACHE_PREFIX = 'minitrack-pois-v2:';
  ...
  function selectedClauses(bbox) {
    ...
    if ($('almsChk')?.checked) {
      ...
      parts.push(`nwr["place"="locality"]["name"](${bbox});`);
    }
  ...
  function visibleBboxes() {
    ...
    const rows = Math.min(4, Math.max(1, Math.ceil(latSpan / 1.0)));
    const cols = Math.min(4, Math.max(1, Math.ceil(lonSpan / 1.0)));
    const boxes = [];
  ...
  async function loadAllVisiblePois(force=false) {
    ...
    // Sofort das anzeigen, was die geladenen Kartenkacheln bereits enthalten.
    try{ tileUpdatePois(); }catch{}
    const seen=new Set();
    for(const cat of Object.keys(markers)) for(const m of markers[cat]){
      const c=m.getLngLat();
      seen.add(cat+'|'+(m.getElement()?.title||'')+'|'+c.lng.toFixed(5)+'|'+c.lat.toFixed(5));
    }

    // Cache ohne Netz-Wartezeit einblenden.
    ...
    const jobs=boxes.map(async (bbox,i)=>{
      const q=`[out:json][timeout:10];(${selectedClauses(bbox).join('')});out center tags;`;
      try{
