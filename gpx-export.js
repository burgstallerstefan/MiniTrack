(()=>{
  ...
  const safeName=s=>(s||'Outabout Route').replace(/[\\/:*?"<>|]+/g,'-').trim()||'Outabout Route';
  ...
  function downloadGpx(coords,name,kind='route'){
    ...
    const gpx=`<?xml version="1.0" encoding="UTF-8"?>\n<gpx version="1.1" creator="Outabout" xmlns="http://www.topografix.com/GPX/1/1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd"><metadata><name>${xml(clean)}</name></metadata><trk><name>${xml(clean)}</name><type>${type}</type><trkseg>${pts}</trkseg></trk></gpx>`;
  ...
  routeBtn?.addEventListener('click',()=>downloadGpx(routeCoords,routeName||'Outabout Route','route'));

  const saveBtn=document.getElementById('saveTrack');
  saveBtn?.addEventListener('click',e=>{
    e.preventDefault();e.stopImmediatePropagation();
    downloadGpx(trackPts,routeName?routeName+' – gelaufen':'Outabout Tour','track');
  },true);
})();